    ...
    <div className="relative">
      ...
      {open && (
        <>
          ...
          <div className="absolute right-0 mt-2 w-80 bg-surface rounded-xl shadow-lg border z-50 max-h-96 overflow-auto">
            <div className="flex items-center justify-between px-4 py-2 border-b">
              <span className="font-medium text-sm">Notifications</span>
              {nonLues > 0 && <button onClick={toutLu} className="text-xs text-brand underline">Tout marquer lu</button>}
            </div>
            {notifs.length === 0 && <p className="p-4 text-sm text-muted text-center">Aucune notification</p>}
            {notifs.map((n) => (
              <button
                key={n.id}
                onClick={() => ouvrir(n)}
                className={`block w-full text-left px-4 py-2 border-b last:border-0 hover:bg-surface-2 ${n.lu ? '' : 'bg-brand/5'}`}
              >
                ...
                <div className="text-[10px] text-muted mt-0.5">{new Date(n.createdAt).toLocaleString('fr-FR')}</div>
              </button>
            ))}
          </div>
        </>
