    ...
    <div>
      ...
      <div className="grid grid-cols-2 gap-4 mb-6">
        <div className="bg-surface rounded-xl p-5 shadow-sm">
          <div className="text-sm text-muted">{t('Revenus perçus (cumulés)', 'Total earnings (paid)')}</div>
          <div className="text-3xl font-bold text-brand mt-1"><MoneyMono value={stats.revenusCumules} /></div>
        </div>
        <div className="bg-surface rounded-xl p-5 shadow-sm">
          <div className="text-sm text-muted">{t('En attente de paiement', 'Awaiting payment')}</div>
          <div className="text-3xl font-bold text-warning mt-1"><MoneyMono value={stats.revenusEnAttente} /></div>
        </div>
      </div>
      <div className="flex items-center justify-between mb-2">
        ...
        <button
          ...
          className="border border-line rounded px-3 py-1.5 text-sm hover:bg-surface-2"
        >
      ...
      <div className="bg-surface rounded-xl shadow-sm overflow-x-auto">
        <table className="w-full text-sm">
          <thead className="bg-surface-2 text-left text-muted"><tr><th className="p-3">{t('Référence', 'Reference')}</th><th className="p-3">{t('Période', 'Period')}</th><th className="p-3">{t('Montant', 'Amount')}</th><th className="p-3">{t('Statut', 'Status')}</th><th className="p-3"></th></tr></thead>
          <tbody>
            {stats.fiches.map((f) => (
              <tr key={f.id} className="border-t border-line">
                ...
                <td className="p-3 font-medium"><MoneyMono value={Number(f.montantTotal)} /></td>
                <td className="p-3"><StatutBadge kind="fiche" value={f.statut} /></td>
            ...
            {stats.fiches.length === 0 && <tr><td className="p-6 text-center text-muted" colSpan={5}>{t('Aucune fiche', 'No slips')}</td></tr>}
          </tbody>
        </table>
      </div>
    </div>
...
function Card({ label, value, alerte }: { label: string; value: string | number; alerte?: boolean }) {
  return (
    <div className="bg-surface rounded-xl p-5 shadow-sm">
      ...
      <div className={`text-3xl font-bold font-mono tabular-nums mt-1 ${alerte ? 'text-danger' : 'text-brand'}`}>{value}</div>
    </div>
  );
}
...
export default function DashboardPage() {
  ...
  if (!stats) return <p className="text-muted">{t('Chargement…', 'Loading…')}</p>;
  ...
  return (
    <div>
      ...
      {!isJri && (
        <div className="bg-surface rounded-xl p-5 shadow-sm mt-6">
